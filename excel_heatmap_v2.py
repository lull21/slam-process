...
def read_main_data(
    ...
    if isinstance(df_obj, dict):
        needed_cols = [ue_col, bs_col, rss_col, flag_col]
        chosen_df = None
        chosen_sheet = None
    ...
    for col in [ue_col, bs_col, rss_col, flag_col]:
        if col not in df.columns:
            raise KeyError(f"主数据缺少必要列: {col}")

    # 只保留相关列并重命名为简洁英文
    df = df[[ue_col, bs_col, rss_col, flag_col]].rename(
        columns={ue_col: "ue_beam", bs_col: "bs_beam", rss_col: "rss", flag_col: "flag"}
    )
    ...
    # 去掉包含NaN的行
    before = len(df)
    df = df.dropna(subset=["ue_beam", "bs_beam", "rss", "flag"])
    after_dropna = len(df)
