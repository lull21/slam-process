...
def read_main_data(
    ...
    if isinstance(df_obj, dict):
        needed_cols = [ue_col, bs_col, rss_col] + ([flag_col] if flag_col else [])
        chosen_df = None
        chosen_sheet = None
    ...
    for col in [ue_col, bs_col, rss_col] + ([flag_col] if flag_col else []):
        if col not in df.columns:
            raise KeyError(f"主数据缺少必要列: {col}")

    # 只保留相关列并重命名为简洁英文
    select_cols = [ue_col, bs_col, rss_col] + ([flag_col] if flag_col else [])
    rename_map = {ue_col: "ue_beam", bs_col: "bs_beam", rss_col: "rss"}
    if flag_col:
        rename_map[flag_col] = "flag"
    df = df[select_cols].rename(columns=rename_map)
    ...
    if "flag" in df.columns:
        df["flag"] = df["flag"].apply(_to_int_safe)
    # RSSI 默认作为浮点数
    ...
    # 去掉包含NaN的行
    before = len(df)
    drop_subset = ["ue_beam", "bs_beam", "rss"] + (["flag"] if "flag" in df.columns else [])
    ...
    # 仅保留 FLAG==1 的有效数据（若存在FLAG列）
    if "flag" in df.columns:
        df = df[mask_valid & (df["flag"] == 1)]
    else:
        df = df[mask_valid]
    after_valid = len(df)
    ...
    if logger:
        logger.info(
            f"原始行数: {before}, 去NaN后: {after_dropna}, 过滤FLAG与波束有效后: {after_valid}"
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--input",
        type=str,
        default=r"D:\Code\SLAMPro\UE21\debugDoc\Serial Debug 2026-01-26 165358_filtered.xlsx",
        help="主数据Excel路径",
    )
    parser.add_argument(
        "--mapping",
