    ...
    def estimate_paths_nn_omp(self, max_paths=10, min_power_ratio=0.01):
        ...
        for k in range(max_paths):
            ...
            print(f"迭代 {k}: AoA={self.aoa_grid[idx_aoa]:.1f}°, AoD={self.aod_grid[idx_aod]:.1f}°, "
                  f"功率={coeffs[-1]:.4f}, 残差={residual_norm:.4f}")
...
class AdvancedPathClassifier:
    ...
    def _identify_nlos_paths(self):
        ...
        NLoS判定标准（AND逻辑）：
        1. 功率条件：比LoS低至少 nlos_power_thresh_db（但不能太弱）
        2. 几何条件：与LoS的角度距离超过一定阈值（同时考虑AoD和AoA）
        3. 去重条件：与已识别NLoS的角度距离足够大
        
        关键：即使在旁瓣区域内，只要满足功率和几何条件，也判定为NLoS
        """
        print(f"\n【步骤2：NLoS径识别】")
        print(f"  NLoS功率阈值: -{self.nlos_power_thresh_db} dB (相对LoS)")
        ...
        for idx, path in candidate_paths.iterrows():
            ...
            # 只要总的角度距离足够大，即使在某个维度的旁瓣区域内也可以是NLoS
            ...
            if is_valid_power and is_geometrically_distinct and is_well_separated_from_nlos:
                ...
                print(f"    功率比={power_ratio_db:.1f}dB, "
                ...
                if not is_valid_power:
                    if power_ratio_db > -self.nlos_power_thresh_db:
                        reasons.append(f"功率过强({power_ratio_db:.1f}dB)")
                    else:
                        reasons.append(f"功率过弱({power_ratio_db:.1f}dB)")
    ...
    def _identify_sidelobe_interference(self):
        ...
        旁瓣干扰特征：
        1. 在旁瓣区域内（与LoS共享AoD或AoA）
        2. 功率相对较弱（比LoS低至少 sidelobe_power_ratio_db）
        3. 不满足NLoS的条件（已被NLoS识别阶段排除）
        """
        print(f"\n【步骤3：旁瓣干扰识别】")
        print(f"  旁瓣区域: AoD ±{self.sidelobe_width_aod}°, AoA ±{self.sidelobe_width_aoa}°")
        print(f"  旁瓣功率阈值: <-{self.sidelobe_power_ratio_db} dB")
        ...
        for idx, path in candidate_paths.iterrows():
            ...
            if in_sidelobe_region and is_weak:
                ...
                print(f"  路径 {idx}: AoD={path['AoD']:.1f}°, AoA={path['AoA']:.1f}° "
                      f"({'+'.join(direction)}, {power_ratio_db:.1f}dB) → 旁瓣干扰")
...
def improved_plot(paths_df, estimator, processor, classifier,
    ...
    #                      linestyle='--', alpha=0.15, label='AoD旁瓣区域')
    #     ax.add_patch(rect1)
    ...
    #                      linestyle='--', alpha=0.15, label='AoA旁瓣区域')
    #     ax.add_patch(rect2)
    ...
    #               c='orange', marker='o', s=120, edgecolors='white',
    #               linewidth=1.5, label='旁瓣干扰', alpha=0.6, zorder=7)
    
    # # 噪声 - 灰色叉号
    # if show_noise and not noise_paths.empty:
    #     ax.scatter(noise_paths['AoD'], noise_paths['AoA'],
    #               c='gray', marker='x', s=100, linewidths=2,
    #               label='噪声/无效', alpha=0.4, zorder=5)
    
    # 6. 设置图表属性
    ax.set_xlabel('出发角 (AoD) [度]', fontsize=14, fontweight='bold')
    ax.set_ylabel('到达角 (AoA) [度]', fontsize=14, fontweight='bold')
    
    title = f'mmWave Multipath Heatmap & Estimation Results\n'
...
def main():
    ...
    try:
        print("=" * 80)
        print("改进的多径估计与分类系统 v2.0")
        print("核心改进：NLoS可以在旁瓣区域内")
        print("=" * 80)
        ...
        # 3. 估计多径
        print("\n步骤 3/4: 执行多径估计...")
        paths = estimator.estimate_paths_nn_omp(max_paths=50, min_power_ratio=0.003)
        ...
        classifier = AdvancedPathClassifier(
            paths,
            sidelobe_width_aoa=3.2,        # 旁瓣宽度
            sidelobe_width_aod=3.2,
            nlos_power_thresh_db=0.15,       # NLoS最小功率差（相对LoS）
            nlos_angle_separation=15,     # NLoS之间的最小角度分离
            sidelobe_power_ratio_db=0.25    # 旁瓣干扰功率阈值
        )
